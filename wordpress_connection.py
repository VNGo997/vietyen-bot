...
class WPClient:
    ...
    def upload_media_from_url(self, image_url: str, filename: str = "hero.jpg"):
        ...
        try:
            img = requests.get(image_url, timeout=30); img.raise_for_status()
        except Exception as e:
            print(f"[WARN] download image fail: {e}"); return None
    ...
    def post_article(self, title: str, content_html: str, status: str = "draft", category_id=None, tag_ids=None, featured_media=None):
        ...
        if r.status_code >= 400: raise RuntimeError(f"WP post failed: {r.status_code} - {r.text}")
        return r.json()

def get_wp_from_env():
    base_url, username, app_pw = os.getenv("WP_URL"), os.getenv("WP_USERNAME"), os.getenv("WP_APP_PASSWORD")
    if not all([base_url, username, app_pw]): raise EnvironmentError("Missing WP_URL, WP_USERNAME or WP_APP_PASSWORD.")
